...
async def msg_handler(client: Client, message: Message):
    ...
    if st == "wait_target":
        await cleanup()
        t_name, t_link = await resolve_channel_info(client, text)
        set_state(uid, "wait_source", json.dumps({"id": text.strip(), "name": t_name, "link": t_link}))
        target_display = format_channel_display(text.strip(), t_name, t_link)
        await send_msg(client, uid,
            f"<b>📣 Now Send Source Channel ID</b>\n\n"
            f"<blockquote><b>Binding to Target: {target_display}</b></blockquote>",
    ...
    if st == "wait_source":
        ...
        s_name, s_link = await resolve_channel_info(client, text)
        pairs = get_prop(f"channels_{uid}", [])
        pairs.append({
            "target": target_info["id"], "target_name": target_info.get("name"), "target_link": target_info.get("link"),
            "source": text.strip(), "source_name": s_name, "source_link": s_link,
        })
        set_prop(f"channels_{uid}", pairs)
        set_state(uid, "")
...
async def process_channel_post(client: Client, message: Message, media_type: str, file_id: str):
    ...
    def _prepare_targets():
        ...
        for aid in admins:
            ...
            for pair in pairs:
                ...
                targets.append((mapped_target, mapped_source, unique_id, btn_text))
    ...
    for mapped_target, mapped_source, unique_id, btn_text in targets:
