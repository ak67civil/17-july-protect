...
def extract_topic(caption: str):
    ...
    Captions often use stylized Unicode fonts for labels (e.g. small-caps
    'ᴛᴏᴘɪᴄ:'), which are completely different codepoints from plain ASCII
    'Topic:' and would never match a plain regex. NFKC normalization folds
    those styled Latin letters back to plain ASCII before matching — it
    leaves non-Latin scripts (Hindi/Devanagari etc.) untouched, so this is
    safe regardless of what language the topic value itself is in.
    """
    if not caption or not caption.strip():
        return None, "Untitled"
    for line in caption.strip().split("\n"):
        line = unicodedata.normalize("NFKC", line.strip())
        m = re.match(r"(?i)^(?:topic|विषय)\s*[:：]\s*(.+)$", line)
        if m:
            val = m.group(1).strip()
            if "→" in val:
                ...
                return parent, child
            return None, val
    first_line = unicodedata.normalize("NFKC", caption.strip().split("\n")[0].strip())
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if d == "add_pair_start":
        ...
        await send_msg(client, uid,
            "<b>🎯 Step 1: TARGET Channel</b>\n\n"
            "<blockquote><b>This is the channel where YOU will upload/post "
            "the raw video.</b> The bot watches this channel — nothing "
            "public happens here, it's just your upload point.\n\n"
            "Forward any post from that channel here (recommended), or "
            "send its Channel ID (starting with -100).</blockquote>",
    ...
    if d.startswith("idx_gen_"):
        ...
        def norm(s):
            # Trim + collapse whitespace + casefold, so "History", "history ",
            # "HISTORY" etc are always recognized as the same topic instead
            # of accidentally creating separate index entries.
        ...
        for e in entries:
            ...
            # Only collapse a video into the previous line when BOTH the
            # group AND the topic are identical to the one right before it
            # — that's a literal repeat (e.g. the same lecture split into
            # several parts). Videos that share a group but have a
            # different topic (e.g. "History → Chauhan Dynasty" then
            # "History → Marwar State") are genuinely different topics and
            # must each get their own line under the shared group heading.
            identity = (norm(group), norm(topic))
...
async def msg_handler(client: Client, message: Message):
    ...
    if st == "wait_target":
        ...
        await send_msg(client, uid,
            f"<b>📣 Step 2: SOURCE Channel</b>\n\n"
            f"<blockquote><b>This is the PUBLIC channel your users/subscribers "
            f"see.</b> The bot will post a protected message with a button "
            f"here — never the raw video itself.\n\n"
            f"Upload point (Target): {target_display}\n\n"
            f"Forward a post from the Source channel (recommended), or send "
            f"its ID.</blockquote>",
    ...
    if st == "wait_source":
        ...
        set_prop(f"channels_{uid}", pairs)
        set_state(uid, "")
        target_display = format_channel_display(target_info["id"], target_info.get("name"), target_info.get("link"))
        source_display = format_channel_display(source_id, s_name, s_link)
        await send_msg(client, uid,
            f"<b>✅ Channels Successfully Linked!</b>\n\n"
            f"<blockquote>🎯 <b>Upload here (Target):</b> {target_display}\n"
            f"📣 <b>Users see it here (Source):</b> {source_display}</blockquote>",
...
async def process_channel_post(client: Client, message: Message, media_type: str, file_id: str):
    ...
    def _prepare_targets():
        ...
        for aid in admins:
            ...
            for pair in pairs:
                ...
                # Topic parsed from a 'Topic : X' or 'Topic : X → Y' line in
                # the caption — saved to the index once the button message
                # actually reaches the Source channel below (that's the
                # message the index needs to link to, not this raw upload).
                ...
                targets.append((mapped_target, mapped_source, unique_id, btn_text, group, topic))
    ...
    for mapped_target, mapped_source, unique_id, btn_text, group, topic in targets:
        ...
        for attempt in range(3):
            try:
                ...
                logger.error(f"Failed to send button to source={mapped_source} "
                             f"(target={mapped_target}): {e}")
                if attempt < 2:
                    await asyncio.sleep(2)

        if sent_msg:
            # Index links to the actual button message the message that
            # landed in the Source channel — that's what users will
            # click, not the raw upload in the Target channel.
            ...
            idx_list.append({
                ...
                "chat_username": sent_msg.chat.username,
            })
