...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if d.startswith("idx_gen_"):
        ...
        for e in entries:
            ...
            # The heading identity is the GROUP when the caption uses the
            # 'Topic : History → Video 3' form — every video in that run
            # shares the group "History" even though the part after the
            # arrow changes per video. Dedupe on that shared identity (or
            # on the topic itself when there's no group), so a run of
            # same-group videos only ever produces ONE line, linking to
            # the first video of that run.
            identity = group if group else topic
            key = norm(identity)
            if key == last_key:
                continue
            last_key = key
